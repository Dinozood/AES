...
def mix_columns(state, operation=ENCRYPT):
    ...
    if operation == ENCRYPT:
        ...
        for i in range(4):
            res_state[0][i] = (
                        gf_mult(0x0E, state[0][i]) ^ gf_mult(0x0B, state[1][i]) ^ gf_mult(0x0D, state[2][i]) ^ gf_mult(
                    0x09, state[3][i]))
            res_state[1][i] = (
                        gf_mult(0x09, state[0][i]) ^ gf_mult(0x0E, state[1][i]) ^ gf_mult(0x0B, state[2][i]) ^ gf_mult(
                    0x0D, state[3][i]))
            res_state[2][i] = (
                        gf_mult(0x0D, state[0][i]) ^ gf_mult(0x09, state[1][i]) ^ gf_mult(0x0E, state[2][i]) ^ gf_mult(
                    0x0B, state[3][i]))
            res_state[3][i] = (
                        gf_mult(0x0B, state[0][i]) ^ gf_mult(0x0D, state[1][i]) ^ gf_mult(0x09, state[2][i]) ^ gf_mult(
                    0x0E, state[3][i]))
